---
type: spec
id: te-pati-maori-economic-development
title: "Māori-led institutions, enterprise and data sovereignty"
status: draft
updated: 2026-08-21
summary: Build Māori-led institutions and funds, support trade and innovation for Māori wealth, and protect Māori data and knowledge.
jurisdiction: NZ
sources:
  - title: Economic development
    url: https://www.maoriparty.org.nz/economic_development
tags:
  - economy
party: te-pati-maori
clusters:
  - maori-affairs
money: no-figure
sourcePath: corpus/nz-election-2026/te-pati-maori/economic_development.md
sourceDigest: "sha256-fbbbfd4a0f0c6987"
gaps:
  - no institutions or funds are named
  - no capitalisation or budget figures
  - trade and innovation supports are not specified
  - data sovereignty mechanisms are not described
  - no timetable
  - no governance model for Māori-led institutions
  - no metrics for Māori wealth building
---

# What the page states

> Māori-led institutions and funds would grow Māori enterprise. Trade, innovation and long-term Māori wealth building would be supported. Māori data and knowledge would be protected through data sovereignty.

System: Māori economic development and data sovereignty

Scenario: Māori-led institutions and funds grow Māori enterprise
Given Māori prosperity cannot rely on systems built to exclude Māori
When Māori-led institutions and funds are built to grow Māori enterprise
Then trade, innovation and long-term Māori wealth building are supported
And Māori data and knowledge are protected through data sovereignty
Outcome Māori thrive economically on their own terms
