---
type: spec
id: kristdemokraterna-var-politik-politik-a-till-o-amorteringskrav
title: Amortization requirements
status: draft
updated: 2026-08-22
summary: Brief information regarding KD's policy regarding amortization requirements.
jurisdiction: SE
sources:
  - title: Amorteringskrav
    url: https://kristdemokraterna.se/var-politik/politik-a-till-o/amorteringskrav?open.12_72d9f8c817e8ce3de028d5b=true
tags:
  - other
party: kristdemokraterna
clusters:
  - economy-tax
money: no-figure
sourcePath: corpus/se-election-2026/kristdemokraterna/var-politik-politik-a-till-o-amorteringskrav.md
sourceDigest: sha256-4349dbf1cff00d86
gaps:
  - cost is not stated on the page
  - timetable is not stated on the page
lang: en
translationOf: kristdemokraterna-amorteringskrav
translator: machine
translatedAt: 2026-08-22
sourceSpecDigest: sha256-fd2d00e03dff864b
---

# What the page states

> <html lang="sv" class="sv-no-js sv-template-bottom">

System: Amorteringskrav

Scenario: Party states this policy
Given the 2026 Swedish general election is contested
When the party publishes this policy
Then <script nonce="fceade81-9e27-11f1-abd6-ff7c5f05d39d">(function(c){c.add('sv-js');c.remove('sv-no-js');})(document.documentElement.classList)</script>
Output Brief information regarding KD's policy regarding amortization requirements.
Outcome the claim is the party's stated position
