---
type: spec
id: act-student-services-fee
title: "Make Student Services Fees voluntary with menu-based selection"
status: draft
updated: 2026-08-21
summary: Amend regulations so universities offer student services as a menu, letting students pay only for chosen services up to the current $1,253 cap instead of a compulsory fee.
jurisdiction: NZ
sources:
  - title: ACT New Zealand
    url: https://www.act.org.nz/student_services_fee
tags:
  - education
  - cost-of-living
party: act
clusters:
  - family-income
money: named-figure
sourcePath: corpus/nz-election-2026/act/student_services_fee.md
sourceDigest: "sha256-ebcde03da21fe1cc"
gaps:
  - which regulations are amended is not named
  - how menu pricing for each service is set is not stated
  - impact on student union funding is not quantified
  - no commencement date is stated
  - how user-pays access works for unchosen services is not detailed
---

# What the page states

> Every domestic university student pays a compulsory Student Services Fee on top of tuition, much of it loan-funded and taxpayer-subsidised. The policy makes fees voluntary through a menu of services students choose and pay for, with unchosen services available on user-pays terms. No regulatory citation or rollout date is given.

System: Voluntary Student Services Fees at universities

Scenario: A domestic university student selects student services each year
Given every domestic university student currently pays a compulsory Student Services Fee of up to $1,253 a year on top of tuition
And across a three-year degree this can add more than $4,000 to a student's loan
And student loans are subsidised by the taxpayer
When the regulations are amended so universities must offer student services as a menu
Then students select the services they want and pay only for those, from fitness facilities to students unions and student media
Output services not chosen still available on a transparent user-pays basis
And up to around $4,000 saved on a student loan over a three-year degree
Outcome students pay only for services they choose
And the taxpayer saves money because student debt is subsidised
And students are not forced to fund unions, advocacy or student media they do not support
