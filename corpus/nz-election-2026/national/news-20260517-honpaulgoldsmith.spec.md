---
type: spec
id: national-news-20260517-honpaulgoldsmith
title: "Abolish good character discounts for sexual offending"
status: draft
updated: 2026-08-21
summary: Judges prohibited from treating good character as a mitigating factor at sentencing for all sexual offending.
jurisdiction: NZ
sources:
  - title: Tougher sentences for sexual offending
    url: https://www.national.org.nz/news/20260517-honpaulgoldsmith
tags:
  - crime-and-justice
party: national
clusters:
  - crime-justice
money: no-figure
sourcePath: corpus/nz-election-2026/national/news-20260517-honpaulgoldsmith.md
sourceDigest: "sha256-5a384fc5bd7e436a"
gaps:
  - legislative timetable is not stated
  - expected sentence length change is not quantified
  - scope of sexual offending categories covered is not listed
  - interaction with the 40 percent sentencing discount cap is referenced but not re-specified here
  - court or prison cost impacts are not stated
---

# What the page states

> Judges would be prohibited from treating good character as a mitigating factor when sentencing all sexual offending. Character testimony from coaches, employers and family could no longer reduce sentences in these cases. The change builds on earlier sentencing discount, name suppression and stalking reforms.

System: Abolishing good character at sentencing for sexual offending

Scenario: A judge sentences a sexual offending case
Given judges must take into account testimony on an offender's character today
When the law prohibits good character as a mitigating factor for all sexual offending
Then judges cannot treat good character as mitigating at sentencing
Outcome tougher sentences for sexual offending
