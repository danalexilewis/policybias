...
import { EVENT_IDS, EVENT_LANGS, type Lang } from '../src/event/events.ts';
...
const NUMBER_TOKEN =
	/\d{1,3}(?:[\u00a0\u2009 ]\d{3})+(?:,\d{1,2})?|\d{1,3}(?:,\d{3})+(?:\.\d+)?|\d+,\d{1,2}(?!\d)|\d+\.\d+|\d+/g;
...
export function canonNumber(raw: string): string {
	...
	if (lastComma >= 0 && lastDot >= 0) {
		token =
			lastComma > lastDot ? token.replace(/\./g, '').replace(',', '.') : token.replace(/,/g, '');
	} else if (lastComma >= 0) {
...
export function checkSpecLanguage(spec: SpecFile, eventId: string): Problem[] {
	const langs =
		eventId === 'nz-election-2026' || eventId === 'se-election-2026'
			? EVENT_LANGS[eventId]
			: undefined;
	if (!langs) {
		return [];
...
function gapsKeyOf(document: GurkiDocument): string | undefined {
	...
	const gaps = extensions.gaps
		.map(String)
		.map((gap) => normaliseProse(gap).toLowerCase())
		.sort();
	if (gaps.length === 0) {
		return undefined;
	}
...
function checkFrontmatter(
	...
	if (extensions.party !== spec.party) {
		fail(
			'party_mismatch',
			`party should be "${spec.party}", found "${String(extensions.party ?? '')}"`
		);
	}
	...
	if (spec.kind === 'stated') {
		...
		if (extensions.assumptions !== undefined) {
			fail(
				'assumptions_on_stated_spec',
				'assumptions belong on the derived spec, not the stated one'
			);
		}
		...
		if (extensions.gaps !== undefined) {
			fail(
				'gaps_on_derived_spec',
				'gaps belong on the stated spec, which is where the page is described'
			);
		}
		...
		if (derivesFrom !== wantedStatedId) {
			fail(
				'derives_from_mismatch',
				`derivesFrom should be "${wantedStatedId}", found "${derivesFrom}"`
			);
		}
		if (!statedIds.has(wantedStatedId)) {
			fail(
				'stated_spec_missing',
				`no stated spec ${spec.slug}.spec.md for this derived spec to read`
			);
		}
	}
	...
	if (page.stance !== 'intervention') {
		fail(
			'not_an_intervention',
			`source page is stance: ${page.stance || 'unset'}, so it should not be modelled`
		);
	}
...
function checkEvent(event: string, options: Options): Problem[] {
	...
	const unmodelled = pages
		...
		.filter(
			(page) => !modelled.has(`${page.party}/${page.slug}`) && !modelledUrls.has(page.sourceUrl)
		);
