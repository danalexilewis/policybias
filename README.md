...
Each finished game can add one anonymous row to a public dataset: the session score, and optional age range, ethnicity, and intended vote. Nothing that identifies a person is stored — no accounts, cookies, names, or IP addresses. The calendar day is kept; the time of day is not.

JSON is at `/api/scores`. CSV is at `/api/scores?format=csv`.

Local `pnpm dev` appends to `data/score-records.jsonl` (gitignored). Production needs a Vercel KV store on the project so the REST env vars are present. Without that, the game still finishes; writes return 503 and the public list is empty.
