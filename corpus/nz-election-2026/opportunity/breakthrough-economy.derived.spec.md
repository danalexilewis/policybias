---
type: spec
id: opportunity-breakthrough-economy-derived
title: "Breakthrough Economy read as a system: a 2% of GDP spend without a dollar total, and break-up powers that still need a court"
status: draft
updated: 2026-08-18
summary: The 2% of GDP research target is a large fiscal commitment whose dollar cost is not in the policy; High Court break-up powers are slow, so the $12 to $31 weekly saving is not a first-year result.
jurisdiction: NZ
sources:
  - title: Breakthrough Economy
    url: https://www.opportunity.org.nz/breakthrough-economy
  - title: Breakthrough Economy policy overview
    url: https://cdn.opportunity.org.nz/documents/policy/breakthrough-economy-policy-overview.pdf
tags:
  - economy
  - small-business
party: opportunity
clusters:
  - tax-fiscal
money: named-figure
sourcePath: corpus/nz-election-2026/opportunity/breakthrough-economy.md
derivesFrom: opportunity-breakthrough-economy
assumptions:
  - 2% of GDP on research is a large annual outlay; without a dollar figure we cannot check it against the land-tax leftover
  - a High Court break-up is a years-long process, so the $12 to $31 weekly saving is a completed-competition figure
  - forgiving student-loan interest for returnees is a fiscal cost and a selection effect toward those who already left
sourceDigest: "sha256-fc532ee34b4a0eb9"
---

# Our understanding

> Research and development would rise to 2% of GDP, with $30 a week lost to uncompetitive sectors and $12 to $31 saved — but 2% of GDP is not converted to dollars and the High Court break-up path is untimed.

System: A research target and a slow competition remedy

Scenario: The research target is funded
Given research and development spending lifted to at least 2% of GDP
When the extra public spend is appropriated
Then the dollar cost has to be found each year
Outcome labs and accelerators grow
But Budget writers still lack a dollar figure to weigh against the Tax Reset leftover
Activates opportunity-tax-reset

Scenario: A duopoly is taken to the High Court
Given break-up powers that run through the High Court
When a supermarket or bank case is filed
Then prices do not move on filing day
Outcome the $12 to $31 weekly saving, if it arrives, arrives after judgment, appeals and a split
But families keep paying the $30 a week the policy names until then
