...
describe('figure extraction', () => {
	...
	it('does not glue neighbouring figures across a comma or space', () => {
		expect(
			extractNumbers('three income-tax tiers at 28% up to $50,000, 34% to $200,000, and 39% above')
		).toEqual(['28', '50000', '34', '200000', '39']);
	});
...
`);
		...
		expect(
			findUnsourcedProse(document, raw, 'Sjukvården behöver inte bara resurser').length
		).toBeGreaterThan(0);
	});
...
`);
		...
		expect(proseCoverage('vanliga människor ska inte behöva stanna inne', page)).toBeGreaterThan(
			0.5
		);
...
Then 12 agencies merge`)
		...
		expect(findUnsourcedFigures(document, page).map((figure) => figure.value)).toEqual([
			'78',
			'12',
		]);
	});
...
describe('resolveActivates', () => {
	...
	function card(id: string, activates: string[], derivedActivates?: string[]): PolicyCard {
		return {
			...
			...(derivedActivates ? { derived: face(`${id}-derived`, derivedActivates, 'derived') } : {}),
...
describe('the authored spec tree', () => {
	...
	it.each(specs)(
		'$party/$name states every figure on its sourced pages',
		({ party, path, kind }) => {
			const document = documentOf(path);
			const extensions = (document.frontmatter?.extensions ?? {}) as Record<string, unknown>;
	...
	it('pairs every derived spec with a stated spec', () => {
		const stated = new Set(
			specs
				.filter((entry) => entry.kind === 'stated')
				.map((entry) => `${entry.party}/${entry.slug}`)
		);
