---
type: spec
id: green-manifesto-2026-pacific-peoples-derived
title: "Pacific partnership read as a system: visa-free entry and an RSE replacement without a printed cost"
status: draft
updated: 2026-08-21
summary: Visa-free Pacific travel and a seasonal scheme with residency pathways change who can arrive and stay, while climate aid and SPREP support stay uncosted in the policy.
jurisdiction: NZ
sources:
  - title: Pacific Peoples
    url: https://www.greens.org.nz/manifesto_2026_pacific_peoples
tags:
  - foreign-policy
  - immigration
  - climate
party: green
clusters:
  - immigration
money: no-figure
sourcePath: corpus/nz-election-2026/green/manifesto_2026_pacific_peoples.md
derivesFrom: green-manifesto-2026-pacific-peoples
assumptions:
  - visa-free travel without a printed country list or conditions is an open border gate until those rules exist
  - replacing the Recognised Seasonal Employer Scheme with open work rights and residency pathways ends the single-employer tie that the current scheme uses to place workers
  - restored and increased climate aid without a dollar figure is a direction of travel, not a sized transfer
  - supporting independence movements is a diplomatic cost that can strain other relationships; the policy does not name that trade-off
sourceDigest: "sha256-5f5bc6e07373b824"
---

# Our understanding

> The policy names visa-free Pacific travel, more climate aid, tangata moana leadership, an RSE replacement with residency pathways, and more SPREP and High Seas support. It does not name the country list, the aid dollars, or how many seasonal workers gain residence.

System: Pacific access and an unpriced seasonal pathway

Scenario: A Pacific visitor arrives without a visa
When visa-free travel is extended to visitors from Pacific Islands
Then arrival no longer depends on a visa grant for those countries
Outcome more short-stay movement is possible
But without a printed country list or conditions, the gate is unnamed

Scenario: Seasonal work is no longer tied to one employer
When the Recognised Seasonal Employer Scheme is replaced with fair wages, housing, open work rights and residency pathways
Then a seasonal worker can leave an employer without losing the right to work
Outcome the labour shortage can still be filled
But residency pathways turn a seasonal inflow into a settlement path the policy does not size

Scenario: Climate aid is restored without a figure
When international climate aid funding is restored and increased
Then Pacific climate negotiations receive more support than today
Outcome solidarity is the named direction
But the transfer cannot be compared with other aid without a printed amount
