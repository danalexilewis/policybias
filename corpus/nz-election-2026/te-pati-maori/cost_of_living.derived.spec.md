---
type: spec
id: te-pati-maori-cost-of-living-derived
title: "Cost of living read as a system: income, welfare and transport relief with rates unnamed"
status: draft
updated: 2026-08-21
summary: Lifting incomes, strengthening welfare and easing transport name three levers; without rates, thresholds or instruments, no household can tell what changes in their pocket.
jurisdiction: NZ
sources:
  - title: Cost of living
    url: https://www.maoriparty.org.nz/cost_of_living
tags:
  - cost-of-living
  - social-welfare
  - transport
party: te-pati-maori
clusters:
  - family-income
money: no-figure
sourcePath: corpus/nz-election-2026/te-pati-maori/cost_of_living.md
derivesFrom: te-pati-maori-cost-of-living
assumptions:
  - an income lift needs a wage floor, a tax change, a transfer, or another named instrument before anyone is paid more
  - strengthening welfare so whānau are not punished for being poor implies which conditions, sanctions or abatement rules change
  - transport affordability is a fare, a subsidy, a concession or a service change; none is named
  - three levers claimed together still need a split of fiscal cost the policy does not give
  - we introduce no figure, because the policy contains none to work from
sourceDigest: "sha256-d1acb086a0c0d11a"
---

# Our understanding

> The policy lifts incomes, strengthens welfare, and backs practical relief including transport affordability. It names no rates, thresholds or instruments.

System: Three relief levers without rates

Scenario: Incomes are to be lifted
Given whānau cannot meet basics across kai, rent, power, transport, medicine and school costs
When incomes are lifted so people can live with dignity
Then a wage, tax or transfer instrument and a rate must still be chosen
Outcome dignity is the aim
But no household can tell from this policy what they would receive

Scenario: Welfare and transport relief land together
When welfare is strengthened so whānau are not punished for being poor
And practical cost-of-living relief includes transport affordability
Then which sanctions or abatement rules change, and which transport instrument applies, remain unnamed
Outcome the direction is less punishment and cheaper travel
But without rates and instruments, neither lever can be sized
