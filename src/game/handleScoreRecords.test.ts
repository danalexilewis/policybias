...
import { memoryScoreRecordStore, unavailableScoreRecordStore } from './scoreRecordStore';
...
describe('handleScoreRecordsRequest', () => {
	...
	it('renders a public html table for browser visits to the event scores page', async () => {
		...
		expect(html).toContain('Guess the party from the policy, not the colour.');
		expect(html).toContain('/nz-election-2026/scores?format=json');
		expect(html).toMatch(/<footer>\s*<a href="\/nz-election-2026\/llms\.txt">llms\.txt<\/a>/);
	...
	it('does not call the store for an invalid payload', async () => {
		...
		await handleScoreRecordsRequest(post({ correct: -1, attempted: 1 }), store, NOW);
