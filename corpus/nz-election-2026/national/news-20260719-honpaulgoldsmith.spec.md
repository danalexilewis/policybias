---
type: spec
id: national-news-20260719-honpaulgoldsmith
title: "Modernise consent laws to require active yes"
status: draft
updated: 2026-08-21
summary: Consent must be actively given with a yes rather than absence of a no, communicated by words or actions.
jurisdiction: NZ
sources:
  - title: National will modernise consent laws
    url: https://www.national.org.nz/news/20260719-honpaulgoldsmith
tags:
  - crime-and-justice
party: national
clusters:
  - crime-justice
money: no-figure
sourcePath: corpus/nz-election-2026/national/news-20260719-honpaulgoldsmith.md
sourceDigest: "sha256-78f47b234ebcc496"
gaps:
  - legislative timetable is not stated
  - how the change affects evidential burden in practice beyond the stated non-shift is not detailed
  - education or prevention programme funding is not stated
  - definitions for intoxication or incapacity are not restated on this page
  - appellate guidance process is not stated
---

# What the page states

> Sexual consent law would require active consent—a yes communicated by words or actions, not merely the absence of a no. Silence would not count as consent, and consent given once would not carry forward to later encounters. The prosecution would still need to prove beyond reasonable doubt that consent was not given.

System: Modernised sexual consent laws

Scenario: Sexual consent is assessed under the Crimes Act
Given the Crimes Act lists what consent is not but does not define consent
When the law on sexual consent is changed
Then consent must be actively given with a yes rather than absence of a no
Output consent communicated by words or actions
Outcome silence is not consent
And consent given once is not consent forever
But prosecution must still prove beyond reasonable doubt that consent was not given
