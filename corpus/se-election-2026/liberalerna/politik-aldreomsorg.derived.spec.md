---
type: spec
id: liberalerna-politik-aldreomsorg-derived
title: "Val i äldreomsorgen läst som system: en meny som fortfarande behöver personal"
status: draft
updated: 2026-08-23
summary: Val och självbestämmande finns bara där en andra utförare och en reservvårdare finns; sidan höjer ambitionen utan ett bemanningsgolv.
jurisdiction: SE
sources:
  - title: Äldreomsorg – Liberalerna
    url: https://www.liberalerna.se/politik/aldreomsorg
tags:
  - elderly-care
derivesFrom: liberalerna-politik-aldreomsorg
party: liberalerna
clusters:
  - elderly-welfare
money: no-figure
sourcePath: corpus/se-election-2026/liberalerna/politik-aldreomsorg.md
sourceDigest: "sha256-e46a87ed412c31c3"
assumptions:
  - valet är tomt i en kommun som bara har ett hem eller ett hemtjänstlag
  - bättre arbetsvillkor utan siffra på lön eller tid skapar inte i sig det extra skift valet kräver
  - ensamhet minskar när någon kommer; det besöket är en arbetstimme sidan inte finansierar
  - självbestämmande över dagen krockar med schemaläggningens effektivitet om inte kortare, dyrare besök accepteras
---

# Our understanding

> Val är en andra dörr. Där bara en utförare finns har menyn en rätt. Bättre villkor för personalen är hur den andra dörren hålls öppen, och de villkoren är oprissatta. Ensamhet är ett besök som någon måste betalas för att göra.

System: Ambition utan rullande schema

Scenario: En äldre person erbjuds val på en tunn marknad
Given valfriheten i äldreomsorgen ska stärkas och skyddas
When personen väljer en annan utförare
Then vardagen ska följa de egna besluten
Outcome självbestämmandet håller bara om ett annat lag kan ta kontraktet
But många kommuner har inget andra lag att välja

Scenario: Bemanningen ska både skära ensamhet och höja kvalitet på en gång
Given arbetsvillkor och kompetens i äldreomsorgen måste förbättras
And ofrivillig ensamhet ska bekämpas
When samma vårdare sägs stanna längre och samtidigt täcka en granne
Then kvalitet och sällskap tävlar om samma timme
Outcome levnadsvillkor utlovas på en högre nivå
But den extra timmen står inte på sidan
