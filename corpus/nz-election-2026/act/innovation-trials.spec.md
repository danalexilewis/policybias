---
type: spec
id: act-innovation-trials
title: "Permanent Innovation Trial process to set aside rules for limited tests"
status: draft
updated: 2026-08-18
summary: Publish a permanent application process letting companies test autonomous vehicles, agricultural drones and medical technology by setting aside a specific regulation for a limited location with safety conditions.
jurisdiction: NZ
sources:
  - title: ACT New Zealand
    url: https://www.act.org.nz/innovation-trials
tags:
  - economy
party: act
clusters:
  - tax-fiscal
money: named-figure
sourcePath: corpus/nz-election-2026/act/innovation-trials.md
sourceDigest: "sha256-995af0010b1f96cb"
gaps:
  - how long a trial may run is not capped on the page
  - criteria for approving or rejecting applications are not stated
  - how the Ministry for Regulation coordinates agencies is not detailed
  - no list of regulations eligible for set-aside is stated
  - cost of running the application process is not stated
---

# What the page states

> The permanent application process, rule set-aside for limited location tests, safety conditions, Ministry for Regulation role, and invitation to autonomous vehicle, drone and med-tech companies are all on the source page. The $2,000 certification cost, one-year timeline, 25 kg and 150 kg drone weight comparisons, and two-year work programme reference appear in a quoted speech on the source page.

System: Innovation Trials for technologies blocked by old rules

Scenario: A company applies to run an Innovation Trial in New Zealand
Given agricultural drones over 25 kilograms can cost up to $2,000 to certify and take over a year in New Zealand
And Australian farmers can use drones weighing up to 150 kilograms under a simple licence
And New Zealand officials have put the issue on a two-year work programme
When a permanent application process for Innovation Trials is published
Then a specific regulation may be set aside for a limited test in one location
Output safety conditions and a way to measure the result set before a trial begins
And the Ministry for Regulation helps agencies identify existing exemption powers and run trials consistently
Outcome any company can apply through one public process instead of finding the right officials
And bad results end the trial while good results give evidence to change the rule
And companies working on autonomous vehicles, agricultural drones and medical technology are invited to run trials in New Zealand
