---
type: spec
id: te-pati-maori-seabed-mining-banned-in-aotearoa-under-reintroduced-member-s-bill
title: "Ban seabed mining across CMA, EEZ and continental shelf including fast-track"
status: draft
updated: 2026-08-21
summary: A reintroduced Members Bill would close all legal pathways for seabed mining, including fast-track approvals, across the coastal marine area, exclusive economic zone and continental shelf.
jurisdiction: NZ
sources:
  - title: Seabed mining banned in Aotearoa under reintroduced Member’s Bill
    url: https://www.maoriparty.org.nz/seabed_mining_banned_in_aotearoa_under_reintroduced_member_s_bill
tags:
  - environment
  - maori-affairs
party: te-pati-maori
clusters:
  - environment-rural
money: no-figure
sourcePath: corpus/nz-election-2026/te-pati-maori/seabed_mining_banned_in_aotearoa_under_reintroduced_member_s_bill.md
sourceDigest: "sha256-b3ade64cbdea7220"
gaps:
  - transition for existing consents is not described
  - enforcement and penalties are not stated
  - no fiscal cost
  - ballot timing beyond "this week" is not a legislative timetable
---

# What the page states

> The Seabed Mining Prohibition Act would close all legal pathways for seabed mining, including fast-track approvals, across the coastal marine area, the exclusive economic zone and the continental shelf. The Bill is prepared for the Members' Bill ballot. No dollar amounts or dates are given.

System: Permanent ban on seabed mining including fast-track

Scenario: Parliament considers a reintroduced Members Bill to ban seabed mining
Given seabed mining has been found to cause unacceptable harm even under fast-track legislation
When the Seabed Mining Prohibition Act is considered
Then all legal pathways for seabed mining are closed, including fast-track approvals
And the ban applies across the coastal marine area, the exclusive economic zone, and the continental shelf
Outcome moana is protected with certainty against seabed mining
Activates te-pati-maori-permanently-ban-seabed-mining
