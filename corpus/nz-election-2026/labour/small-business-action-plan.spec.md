---
type: spec
id: labour-small-business-action-plan
title: "Paid on time, $10,000 write-off, and $80,000 GST threshold"
status: draft
updated: 2026-08-21
summary: Large businesses pay small suppliers within 15 days, a $10,000 asset write-off from July 2027, and GST registration threshold lifted to $80,000 from July 2028.
jurisdiction: NZ
sources:
  - title: Small Business Action Plan
    url: https://www.labour.org.nz/small-business-action-plan
tags:
  - small-business
  - taxation
party: labour
clusters:
  - tax-fiscal
money: named-figure
sourcePath: corpus/nz-election-2026/labour/small-business-action-plan.md
sourceDigest: "sha256-32a2741f0e85e3e1"
gaps:
  - penalty amounts for late payment are not stated beyond proportionate fines
  - large business revenue and expenditure thresholds for the paid-on-time rule are stated but verification process is not
  - write-off eligible asset list beyond examples is not exhaustive
  - GST threshold savings to government are not stated
  - how Investment Boost is fully wound down is not detailed
---

# What the page states

> Large businesses must pay small suppliers within 15 days for smaller invoices and publish payment times. Small businesses can write off new equipment purchases from 1 July 2027. The GST registration threshold rises from 1 July 2028.

System: Small Business Action Plan

Scenario: A small business receives an invoice from a large business
Given large businesses can currently pay small suppliers late
When paid-on-time legislation is introduced within the first 100 days
Then big companies must pay small businesses on time and publish how quickly they pay
Output payment within 15 days for invoices of $25,000 or less
And large businesses must publish how quickly they pay
And small businesses can write off up to $10,000 in new equipment
And no compulsory GST registration until $80,000
And $1.56 billion cost over four years
And $1.355 billion for the asset write-off over four years
Outcome around 500,000 businesses eligible for the asset write-off
And around 35,000 businesses no longer required to register for GST
